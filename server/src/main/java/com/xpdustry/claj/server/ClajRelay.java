...
public class ClajRelay extends Server {
    ...
    public String generateLink() {
        StringBuilder builder = new StringBuilder(linkPrefix);
    ...
    public class Listener implements NetListener {
        ...
        public void connected(Connection connection) {
            ...
            Log.info("Connection @ received!", connectionIDToString(connection));
            connection.setArbitraryData(new Ratekeeper());
        }

        @Override
        public void disconnected(Connection connection, DcReason reason) {
            Log.info("Connection @ lost: @.", connectionIDToString(connection), reason);
        ...
        public void received(Connection connection, Object object) {
            ...
            if (!rate.allow(3000L, spamLimit)) {
                ...
                if (redirector != null && connection == redirector.host) {
                    Log.warn("Connection @ spammed with packets but not disconnected due to being a host.", 
                             connectionIDToString(connection));
                    return; // host can spam packets when killing core and etc.
                }

                Log.warn("Connection @ disconnected due to packet spam.", connectionIDToString(connection));
                if (redirector != null) {
            ...
            if (object instanceof String link) {
                if (link.equals("new")) {
                    ...
                    Log.info("Connection @ hosted a redirector in room @.", connectionIDToString(connection), room.link);
                    ...
                    Log.info("Connection @ joined to room @.", connectionIDToString(connection), room.link);
                }
